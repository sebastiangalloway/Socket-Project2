...
class Socket:
    ...
    def _recv(self):
        ...
        try:
            #previously 1024 doesnt go to last from address if each packet is 424 byte size
            (inPacket, self.lastFromAddr) = self.sock.recvfrom(MAX_PACKET_SIZE)
    ...
    def sendSynPacket(self):
        ...
        #If SYN is present the sequence number is the initial sequence number (ISN) and the first data octet is ISN+1. true above
        #S (SYN, 1 bit): Synchronize sequence numbers (Confundo connection establishment)
        #syn packet sends 1 bit which resets back to 0
    ...
    def bulkSend(self, retransmit):
        if retransmit:
            self.seqNum = self.base
        data_sent = modSubtract(self.seqNum, self.base)
        bytes_sent = 0
        while True: 
            toSend = self.outBuffer[data_sent: data_sent + MTU_SIZE]
    ...
    def send(self, data):
        ...
        while len(self.outBuffer) > 0:
            bytes_sent = self.bulkSend(retransmit)
            if bytes_sent > 0:
                retransmit = False
                startTime = time.time()
            ...
            if time.time() - startTime > RETX_TIME:
                self.cc.on_timeout()
                retransmit = True
